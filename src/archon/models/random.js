...
const procTypes = ["web", "service", "oneshot"];
const images = ["ubuntu", "centos", "golang", "busybox", "training/webapp"];
const procStatus = ["ok", "ok", "ok", "ok", "ok", "fail", "fail", "pending", "pending", "drifting"];

export let RandomApp = function() {
  let appName = choice(appNames);
  let procs = [];
  for (let i = 0; i < Math.random() * 10; i += 1) {
    let procType = choice(procTypes);
    let proc = {
      name: choice(procNames),
      proctype: procType,
      image: choice(images),
      num_instances: Math.floor(Math.random() * 10) + 1,
      cpus: Math.floor(Math.random() * 5) + 1,
      memory: Math.floor(Math.random() * 15) + 1,
      state: choice(procStatus),
    };
    procs.push(proc);
  }
  let services = [];
  for (let i = 0; i < Math.random() * 3; i += 1) {
    let sName = choice(appNames);
    let service = {
      name: sName,
      procs: [
        `service.${sName}.${choice(procNames)}`,
        `service.${sName}.${choice(procNames)}`,
      ],
    };
  ...
  let app = {
    name: appName,
    meta_version: Math.random().toFixed(6),
    procs: procs,
    services: services,
  }
  return app;
